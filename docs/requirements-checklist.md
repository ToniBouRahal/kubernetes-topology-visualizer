...
| Raw IP logging off by default | ADR-001 §6 | `TestRawEventLoggingIsOffUnlessExplicitlyEnabled` (T-2.13) — 8 cases incl. `1`/`True`/`yes` | ✅ |

## Phase 2 — end-to-end live product

| Req | Source | Resolved by | Status |
|---|---|---|---|
| Idempotent ingestion | ADR-004 D-4.3 | `test_replaying_a_batch_moves_no_counter`, `test_concurrent_replays_are_counted_once` | ✅ |
| Bounded queue, retry with jitter | ADR-002 D-2.6 | `TestQueueOverflowDropsOldest`, `TestRetryableStatuses`, `TestQueueHighWaterMarkIsRecorded` | ✅ |
| Graceful shutdown flush | ADR-002 D-2.6 | `TestShutdownDrainsQueue` | ✅ |
| Nodes derived only from in-window edges | ADR-004 D-4.4 | T-4.9 | ✅ |
| Deterministic ordering | ADR-003 D-3.6 | `test_repeated_graph_queries_have_identical_explicit_edge_order`, `test_output_is_ordered_by_the_edge_key` | ✅ |
| Truncation indicator | ADR-004 D-4.4 | `test_result_is_truncated_with_an_indicator` | ✅ |
...
| Req | Source | Resolved by | Status |
|---|---|---|---|
| Transactional idempotent ingest | ADR-005 D-5.3 | `test_first_ingest_is_recorded`, `test_a_distinct_batch_id_does_accumulate` (both adapters) | ✅ |
| Retries do not double count | ADR-001 §9 | `test_replaying_a_batch_moves_no_counter` (both adapters) | ✅ |
| One-minute buckets | ADR-005 D-5.4 | `tests/test_window.py::TestBucketStart` — 12 cases incl. timezone conversion (T-5.5) | ✅ |
| Half-open `[from, to)` boundaries | ADR-005 D-5.4 | T-5.7 | ✅ |
| Retention + orphan cleanup | ADR-005 D-5.5 | T-5.8 | ✅ |
| History survives restarts | ADR-001 §9 | `test_data_survives_a_new_connection_pool`; observed in-cluster, `evaluation/phase-3.md` | ✅ |
...
| Readable without colour | ADR-006 D-6.3 | `diff.test.ts` "remains distinguishable with colour removed" | ✅ |
| Keyboard reachable, WCAG AA | ADR-006 D-6.7 | T-6.9 | ✅ |
| Usable at 1280×720 | ADR-001 §5.6 | `e2e/topology.spec.ts` "is usable at 1280x720" | ✅ |
| Intensity metric named explicitly | ADR-006 D-6.4 | legend states "connection count — TCP establishments, not requests"; `diff.test.ts` | ✅ |
| Byte accounting delivered **or** documented | ADR-001 §9 | **declined on evidence** — `evaluation/byte-accounting.md`, 5 experiments | 📄 |
...
| Multi-node kubeadm validation | ADR-001 §9 | NetworkPolicy half verified under Calico; separate-kernel half not possible on this host — `limitations.md` §3.4 | 📄 |
| Pod churn does not fragment identity | ADR-001 §9 | T-8.6 | ✅ |
| `demo-down` is surgical | ADR-007 D-7.6 | T-7.11 | ✅ |
| Performance targets measured | ADR-001 §6 | `layout.test.ts` "scale ceiling" — 0.3 ms cached / 208 ms on change; budget missed, tracked as P5-F18 in `limitations.md` §4.1 | 📄 |
| Failure modes actionable | ADR-001 §7 P5 | every scenario triggered live — `evaluation/phase-5.md` P5-T17 | ✅ |
