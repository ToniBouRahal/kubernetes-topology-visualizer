...
- [ ] **P2-B1** API/domain/persistence layering, `settings.py`, app factory — D-4.1 (fixes B1)
- [ ] **P2-B2** Structured JSON logging with request IDs — D-4.6 (fixes B10)
- [ ] **P2-B3** `RepositoryProtocol` + `InMemoryRepository`, injected clock — D-4.2, ADR-005 D-5.1 (fixes B3, B12)
- [ ] **P2-B4** `POST /api/v1/ingest/batches`: 202/200/400/422/413 — D-4.3 (fixes B4, B5) · tests T-4.1 – T-4.3
- [ ] **P2-B5** `GET /api/v1/graph`: presets, filters, deterministic order, truncation — D-4.4 (fixes B7, B9, B11) · tests T-4.4, T-4.7 – T-4.10
- [ ] **P2-B6** `/namespaces`, `/nodes/{id}`, `/edges/{id}` with URL-encoded IDs — D-4.7 (fixes B8) · test T-4.13
