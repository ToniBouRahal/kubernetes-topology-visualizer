...
- [x] **P1-K2** A pod Running on every node of the three-node cluster — test T-7.5
...
- [x] **P5-K10** NetworkPolicies shipped; kind non-enforcement documented — D-7.4 · **the database policy was missing.** D-7.4 requires ingest restricted to agents *and* the database restricted to the backend; only the first existed, so any pod in the namespace could read the full topology on 5432. Both now ship, and are **on by default** since P5-K16 verified them under Calico: every pod reached Ready with the policies enforced — so the kubelet-probe risk did not materialise — and an unlabelled pod is genuinely blocked from 5432 and ingest, which is what proves enforcement rather than permissiveness. The kind demo values keep them off, because objects that do nothing under kindnet would imply a protection that cluster does not provide.
- [x] **P5-K11** Make targets: `demo-up`, `demo-traffic`, `demo-change`, `demo-verify`, `demo-down`, plus `images` — D-7.6 · `demo-verify` asserts 8 properties through the API, including that a burst of exactly 100 connections is reported as exactly 100. `demo-down` scopes every deletion by release name, cluster name, or the `topology-demo` label — never a bare namespace name.
- [x] **P5-K12** Demo workloads across ≥2 namespaces + the controlled change scenario — D-7.7 · **found that the documented `backend → EXTERNAL` edge had never worked in 24h of running**: the manifest used `(echo > /dev/tcp/host/port)`, which is bash-only, in a BusyBox `ash` container, with `|| true` hiding the failure. Replaced with `nc`; the edge now appears.
...
- [~] **P5-K16** Validation under an enforcing CNI — D-7.8 · test T-7.12 · **NetworkPolicy half DONE** on a throwaway Calico cluster: all pods Ready with policies enforced (kubelet probes are not dropped), legitimate paths work, and an unlabelled pod is blocked from both 5432 and ingest. **Separate-kernel half not possible on this host** — kind shares one kernel and a kubeadm pair needs VMs the machine cannot spare. Reproduction steps and the exact check (`topology_agent_events_filtered_foreign_node_total` must read zero) are in `docs/evaluation/phase-5.md`.
