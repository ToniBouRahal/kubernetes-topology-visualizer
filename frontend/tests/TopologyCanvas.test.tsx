...
/*
 * Edges carry no text. The port and every count live in the details panel, one click away; the
 * canvas stays a picture of who talks to whom. Unmeasured-versus-zero is asserted there
 * (DetailsPanel.test.tsx) and in the grouping sums (grouping.test.ts).
 */
it("draws no text on any edge, selected or not", () => {
  const {rerender} = render(<TopologyCanvas graph={graph} selectedId={null} onSelect={() => {}}/>);
  expect(screen.getAllByTestId("edge").map(e => e.textContent)).toEqual(["", ""]);
  rerender(<TopologyCanvas graph={graph} selectedId="a" onSelect={() => {}}/>);
  expect(screen.getAllByTestId("edge").map(e => e.textContent)).toEqual(["", ""]);
});

it("still names each edge's ends and port for a screen reader", () => {
  render(<TopologyCanvas graph={graph} selectedId={null} onSelect={() => {}}/>);
  expect(screen.getByLabelText("a to b, TCP port 80")).toBeInTheDocument();
  expect(screen.getByLabelText("b to c, TCP port 80")).toBeInTheDocument();
});
...
it("draws a failed-only connection as a dashed warning, without text", () => {
